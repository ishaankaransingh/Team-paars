...
if (isset($_SESSION['gebruiker_id']) && isset($_SESSION['user_name'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['naam'], $_POST['voornaam'], $_POST['geboortedatum'])) {
      ...
      $role_id = 4; // Assuming role_id for docenten is 3
...
$totalrcResult = $conn->query("SELECT COUNT(*) as total FROM personen WHERE rol_id =4");
$totalrc = $totalrcResult->fetch_assoc()['total'];

// Fetch online docenten count
$onlinercResult = $conn->query("SELECT COUNT(*) as online FROM personen WHERE rol_id = 4 AND active = 1");
$onlinerc = $onlinercResult->fetch_assoc()['online'];

// Fetch offline docenten count
$offlinerc = $totalrc - $onlinerc;
...
<div class="sidebar" id="sidebar">
    ...
    <h1 class="dashboard-title">Richtingcoordinator Management</h1>

   



        <button class="btn" id="addDocentBtn">Add Richtingcoordinator</button>
...
$result = $conn->query("SELECT p.persoon_id, p.naam, p.voornaam, t.email, t.password, p.active FROM personen p JOIN tgebruiker t ON p.persoon_id = t.persoon_id WHERE p.rol_id = 4");
while ($rc = $result->fetch_assoc()) {
    // Determine the status for the light indicator
    $statusClass = $rc['active'] ? 'green' : 'red'; // 'green' for online, 'red' for offline
    echo "<tr>
        <td>{$rc['naam']}</td>
        <td>{$rc['voornaam']}</td>
        <td>{$rc['email']}</td>
        <td>
            <span class='light $statusClass'></span> " . ($rc['active'] ? 'Yes' : 'No') . "
        </td>
        <td>
            <button class='btn' onclick='openUpdateModal({$rc['persoon_id']}, \"{$rc['naam']}\", \"{$rc['voornaam']}\", \"{$rc['email']}\", \"{$rc['password']}\", {$rc['active']})'>Update</button>
            <form method='POST' style='display:inline;'>
                <input type='hidden' name='delete_id' value='{$rc['persoon_id']}'>
                <button type='submit' class='btn' onclick=\"return confirm('Are you sure you want to delete this rc?');\">Delete</button>
            </form>
        </td>
    </tr>";
}
?>
    ...
    <div class="card">
        <h3>Total Richtingcoordinator</h3>
        <p><?php echo $totalrc; ?></p>
    </div>
    <div class="card">
        <h3>Online Richtingcoordinator</h3>
        <p><?php echo $onlinerc; ?></p>
    </div>
    <div class="card">
        <h3>Offline Richtingcoordinator</h3>
        <p><?php echo $offlinerc; ?></p>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <h2 class="modal-title">Add Richtingcoordinator</h2>
        <div class="form-container">
            <form id="addDocentForm" method="POST">
                ...
                <button type="submit" class="btn">Add Richtingcoordinator</button>
            </form>
        </div>
    </div>
</div>
...
<div id="updateDocentModal" class="modal">
    <div class="modal-content">
        ...
        <h2 class="modal-title">Update Richtingcoordinator</h2>
        <div class="form-container">
            <form id="updateDocentForm" method="POST">
                ...
                <button type="submit" class="btn">Update Richtingcoordinator</button>
            </form>
        </div>
    </div>
</div>
