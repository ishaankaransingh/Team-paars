...
if (isset($_SESSION['gebruiker_id']) && isset($_SESSION['user_name'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['naam'], $_POST['voornaam'], $_POST['geboortedatum'])) {
      ...
      $role_id = 6; // Assuming role_id for docenten is 3
...
$totaldirecteurResult = $conn->query("SELECT COUNT(*) as total FROM personen WHERE rol_id = 6");
$totaldirecteur = $totaldirecteurResult->fetch_assoc()['total'];

// Fetch online docenten count
$onlinedirecteurResult = $conn->query("SELECT COUNT(*) as online FROM personen WHERE rol_id = 6 AND active = 1");
$onlinedirecteur = $onlinedirecteurResult->fetch_assoc()['online'];

// Fetch offline docenten count
$offlinedirecteur= $totaldirecteur - $onlinedirecteur;
...
<head>
    ...
    <title>Directeur Management</title>
...
<div class="sidebar" id="sidebar">
    ...
    <h1 class="dashboard-title">Directeur Management</h1>

   



        <button class="btn" id="addDocentBtn">Add directeur</button>
...
$result = $conn->query("SELECT p.persoon_id, p.naam, p.voornaam, t.email, t.password, p.active FROM personen p JOIN tgebruiker t ON p.persoon_id = t.persoon_id WHERE p.rol_id = 6");
while ($directeur = $result->fetch_assoc()) {
    // Determine the status for the light indicator
    $statusClass = $directeur['active'] ? 'green' : 'red'; // 'green' for online, 'red' for offline
    echo "<tr>
        <td>{$directeur['naam']}</td>
        <td>{$directeur['voornaam']}</td>
        <td>{$directeur['email']}</td>
        <td>
            <span class='light $statusClass'></span> " . ($directeur['active'] ? 'Yes' : 'No') . "
        </td>
        <td>
            <button class='btn' onclick='openUpdateModal({$directeur['persoon_id']}, \"{$directeur['naam']}\", \"{$directeur['voornaam']}\", \"{$directeur['email']}\", \"{$directeur['password']}\", {$directeur['active']})'>Update</button>
            <form method='POST' style='display:inline;'>
                <input type='hidden' name='delete_id' value='{$directeur['persoon_id']}'>
                <button type='submit' class='btn' onclick=\"return confirm('Are you sure you want to delete this directeur?');\">Delete</button>
            </form>
        </td>
    </tr>";
}
?>
    ...
    <div class="card">
        <h3>Total directeur</h3>
        <p><?php echo $totaldirecteur; ?></p>
    </div>
    <div class="card">
        <h3>Online directeur</h3>
        <p><?php echo $onlinedirecteur; ?></p>
    </div>
    <div class="card">
        <h3>Offline directeur</h3>
        <p><?php echo $offlinedirecteur; ?></p>
    </div>
</div>
    ...
    <div class="modal-content">
        ...
        <h2 class="modal-title">Add directeur</h2>
        <div class="form-container">
            <form id="addDocentForm" method="POST">
                ...
                <button type="submit" class="btn">Add directeur</button>
            </form>
        </div>
    </div>
</div>
...
<div id="updateDocentModal" class="modal">
    <div class="modal-content">
        ...
        <h2 class="modal-title">Update directeur</h2>
        <div class="form-container">
            <form id="updateDocentForm" method="POST">
                ...
                <button type="submit" class="btn">Update directeur</button>
            </form>
        </div>
    </div>
</div>
